...
public class MetricNormalizerTest {
  ...
  void testEmptySet() {
    ...
    MetricNormalizer normalizer = new MetricNormalizer(multiset);
  ...
  void testAllZeroMultiset() { // p95=0, max=0 → always 0
    MetricNormalizer n = new MetricNormalizer(new double[] {0.0, 0.0, 0.0});
  ...
  void testSingleElement() { // n=1 → index ⌈0.95⌉-1 = 0
    MetricNormalizer n = new MetricNormalizer(new double[] {4.0});
  ...
  void testTwoElementMultiset() { // the hand-computed {1,3} case
    MetricNormalizer n = new MetricNormalizer(new double[] {1.0, 3.0});
    // n=2, ⌈1.9⌉-1 = 1 → p95 = 3
  ...
  void testTwentyElementsIndexSemantics() { // §10: ⌈0.95·20⌉ = 19 (1-based) → sorted[18]
    ...
    MetricNormalizer n = new MetricNormalizer(v);
  ...
  void testUnsortedInputSortedAndNotMutated() {
    ...
    MetricNormalizer n = new MetricNormalizer(input);
    // n=3, ⌈2.85⌉-1 = 2 → p95 = 3
    assertEquals(1.0, n.normalize(3.0));
