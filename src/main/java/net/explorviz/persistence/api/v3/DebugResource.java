...
public class DebugResource {
  ...
  @Inject DebugRunRepository debugRunRepository;

  @Inject DebugSnapshotRepository debugSnapshotRepository;

  @Inject VariableRepository variableRepository;

  @GET
  @Produces(MediaType.APPLICATION_JSON)
  @Path("/debug-runs/{repositoryName}")
  public List<DebugRunDto> getDebugRuns(
  ...
  public List<DebugSnapshotDto> getDebugSnapshots(@RestPath final String debugRunId) {
    final Session session = sessionFactory.openSession();
    return debugSnapshotRepository.findDebugSnapshotsForDebugRun(session, debugRunId).stream()
        .map(
            data ->
                new DebugSnapshotDto(
                    data.getId().toString(),
  ...
  public List<VariableDto> getVariables(@RestPath final String debugSnapshotId) {
    final Session session = sessionFactory.openSession();
    return variableRepository.findVariablesForDebugSnapshot(session, debugSnapshotId).stream()
        .map(
            data ->
                new VariableDto(
                    data.getId().toString(),
